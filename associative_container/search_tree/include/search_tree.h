...
#include "../../../allocators/allocator/include/allocator.h"
#include "../../../allocators/allocator/include/allocator_guardant.h"
...
protected:
    ...
    common_node * _root = nullptr;

protected:

    common_node *create_node(size_t t) const;

    void destroy_node(common_node *to_destroy);

    int node_find_path(typename search_tree<tkey, tvalue>::common_node const *node, tkey const &key, size_t left_bound_inclusive, size_t right_bound_inclusive);

    void node_insert(typename search_tree<tkey, tvalue>::common_node *node, typename associative_container<tkey, tvalue>::key_value_pair &&kvp, size_t subtree_index, typename search_tree<tkey, tvalue>::common_node *right_subtree);

    std::pair<typename search_tree<tkey, tvalue>::common_node *, typename associative_container<tkey, tvalue>::key_value_pair> node_split(typename search_tree<tkey, tvalue>::common_node *node, typename associative_container<tkey, tvalue>::key_value_pair &&kvp, size_t subtree_index, typename search_tree<tkey, tvalue>::common_node *right_subtree);

    void merge_nodes(typename search_tree<tkey, tvalue>::common_node *parent, size_t left_subtree_index);
...
search_tree<tkey, tvalue>::common_node::common_node(
    typename associative_container<tkey, tvalue>::key_value_pair *keys_and_values, 
    common_node **subtrees, 
    size_t t) : keys_and_values(keys_and_values), subtrees(subtrees), virtual_size(0)
{
    for (size_t i = 0; i < 2 * t; ++i) subtrees[i] = nullptr;
}
...
template<
    typename tkey, 
    typename tvalue>
typename search_tree<tkey, tvalue>::common_node *search_tree<tkey, tvalue>::create_node(size_t t) const
{
    auto *keys_and_values = reinterpret_cast<typename associative_container<tkey, tvalue>::key_value_pair *>(allocate_with_guard(sizeof(typename associative_container<tkey, tvalue>::key_value_pair), 2 * t - 1));
    auto *subtrees = reinterpret_cast<typename search_tree<tkey, tvalue>::common_node **>(allocate_with_guard(sizeof(typename search_tree<tkey, tvalue>::common_node *), 2 * t));
    auto *node = reinterpret_cast<typename search_tree<tkey, tvalue>::common_node *>(allocate_with_guard(sizeof(typename search_tree<tkey, tvalue>::common_node)));
    ...
    typename search_tree<tkey, tvalue>::common_node *right_subtree)
{
    ...
    {
        swap(std::move(pair), std::move(node->keys_and_values[mediant_index - (subtree_index < mediant_index ? 1 : 0)]));
        swap(std::move(right_subtree), std::move(node->subtrees[mediant_index + (subtree_index < mediant_index ? 0 : 1)]));
    }

    auto index = mediant_index;
    int index_increment = index < subtree_index ? 1 : -1;
    
    if (index_increment == -1) --index;
    ...
    return std::make_pair(new_node, std::move(pair));
}

template<
    typename tkey, 
    typename tvalue>
void search_tree<tkey, tvalue>::destroy_node(common_node * to_destroy)
{
    for (size_t i = 0; i < to_destroy->virtual_size; ++i) allocator::destruct(to_destroy->keys_and_values + i);
    ...
    size_t right_bound_inclusive)
{
    ...
    while (true)
    {
        ...
        if (comparison_result == 0) return index;

        if (left_bound_inclusive == right_bound_inclusive) return -(index + (comparison_result < 0 ? 0 : 1) + 1);

        if (comparison_result < 0) right_bound_inclusive = index;
        else left_bound_inclusive = index + 1;
    }
}
...
void search_tree<tkey, tvalue>::node_insert(
    typename search_tree<tkey, tvalue>::common_node * node, 
    typename associative_container<tkey, tvalue>::key_value_pair &&pair, 
    size_t subtree_index, typename search_tree<tkey, tvalue>::common_node *right_subtree)
{
    allocator::construct(node->keys_and_values + node->virtual_size, std::move(pair));
    node->subtrees[node->virtual_size + 1] = right_subtree;

    for (size_t i = 0; i < node->virtual_size - subtree_index; i++)
    {
        swap(std::move(node->keys_and_values[node->virtual_size - i]), std::move(node->keys_and_values[node->virtual_size - i - 1]));
...
template<
    typename tkey, 
    typename tvalue>
std::stack<std::pair<typename search_tree<tkey, tvalue>::common_node **, int>> search_tree<tkey, tvalue>::find_path(tkey const &key)
{
...
template<
    typename T>
inline void search_tree<tkey, tvalue>::swap(T &&one, T &&another)
{
    T tmp = std::move(one);
...
search_tree<tkey, tvalue>::search_tree(
    ...
    _keys_comparer(keys_comparer), _logger(logger), _allocator(allocator)
{}
